...
function buildGroups(viewer){
  ...
  for(const [prop,label] of MAPS){
    ...
    for(const mesh of viewer.meshes){for(const mat of mats(mesh)){const texture=mat?.[prop];if(!texture?.isTexture)continue;let key=texture.userData?.uvmapGroupKey;if(key&&key.startsWith(`${prop}__g`)){let group=byKey.get(key);if(!group){group={key,label,texture,names:new Set()};groups.push(group);byKey.set(key,group);}if(mat.name)group.names.add(mat.name);continue;}let group=byTexture.get(texture);if(!group){key=`${prop}__g${groups.length}`;group={key,label,texture,names:new Set()};groups.push(group);byTexture.set(texture,group);byKey.set(key,group);texture.userData=texture.userData||{};texture.userData.uvmapGroupKey=key;}if(mat.name)group.names.add(mat.name);}}
...
function eachTarget(viewer,key,callback){
  ...
  for(const mesh of viewer.meshes){if(!uvAttr(mesh.geometry,viewer.uvSetName))continue;for(const mat of mats(mesh)){if(!mat)continue;if(grouped&&mat[prop]?.userData?.uvmapGroupKey!==key)continue;callback(mesh,mat,prop);}}
}
